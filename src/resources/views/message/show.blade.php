        ...
        <div class="chat-submit">
            ...
            @if ($purchase->status !== 'completed')
                <form action="{{ route('message.store', $purchase) }}" method="POST" class="trade-form" enctype="multipart/form-data">
                    @csrf
                    <input type="text" name="content" id="chat-input" class="trade-form__content" placeholder="取引メッセージを記入してください" novalidate>

                    {{-- 画像添付 --}}
                    <input type="file" name="image" class="trade-form__image-input" style="display:none;" accept="image/*">
                    <button type="button" class="trade-form__image-btn" onclick="document.querySelector('.trade-form__image-input').click();">
                        画像を追加
                    </button>
...
document.querySelector('form.trade-form').addEventListener('submit', () => {
